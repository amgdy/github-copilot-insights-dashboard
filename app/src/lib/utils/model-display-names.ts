 ...
 *
 * The dim_model table is the source of truth for display names. The static
 * map below is used only as the default for newly-ingested models.
 */

import { db } from "@/lib/db";
import { dimModel } from "@/lib/db/schema";
...
/**
 * Build a display-name resolver from dim_model in the database.
 * Returns a function with the same signature as modelDisplayName()
 * but uses the DB-stored display_name (edited by admins) first.
 */
export async function getModelDisplayMap(): Promise<(rawName: string) => string> {
  ...
  for (const r of rows) {
    if (r.displayName) dbMap.set(r.modelName, r.displayName);
  }

  return (rawName: string): string => {
    return dbMap.get(rawName) ?? modelDisplayName(rawName);
  };
}
...
/**
 * Fetch the GitHub Models Catalog and build a lookup map.
 * Keys are the model-name portion of the catalog id (e.g., "gpt-4o" from "openai/gpt-4o").
 * Values are the catalog display name (e.g., "OpenAI GPT-4o").
 */
export async function fetchModelsCatalog(token: string): Promise<Map<string, { displayName: string; isPremium: boolean }>> {
  ...
  if (!res.ok) {
    throw new Error(`GitHub Models Catalog API error: ${res.status} ${res.statusText}`);
  }

  const models: CatalogModel[] = await res.json();
  const map = new Map<string, { displayName: string; isPremium: boolean }>();

  for (const m of models) {
    // Catalog id is "publisher/model-name", e.g. "openai/gpt-4o"
    const modelName = m.id.includes("/") ? m.id.split("/").slice(1).join("/") : m.id;
    const isPremium = m.rate_limit_tier === "custom";
    map.set(modelName, { displayName: m.name, isPremium });
  }
